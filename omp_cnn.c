...
static void Layer_feedBack_full(Layer* self)
{
    ...
    double dnet[out_size];
    #pragma omp parallel for
    for (int i = 0; i < out_size; i++) {
        dnet[i] = self->errors[i] * self->gradients[i];
        self->u_biases[i] += dnet[i];
    }
    ...
    for (int j = 0; j < in_size; j++) {
        ...
        #pragma omp simd reduction(+:my_error)
        for (int i = 0; i < out_size; i++) {
            int w_idx = i * in_size + j;
            /* Propagate the errors to the previous layer. */
            my_error += self->weights[w_idx] * dnet[i];
            self->u_weights[w_idx] += dnet[i] * lprev->outputs[j];
        }
        lprev->errors[j] = my_error; 
    }
